...
contract BindowsVaultV2Upgradeable is Initializable, UUPSUpgradeable {
    ...
    event DevOwnershipTransferred(address indexed oldDev, address indexed newDev);
    ...
    function initialize() public initializer {
        // __UUPSUpgradeable_init(); -> Removed because UUPSUpgradeable does not define an initializer function
        
        devAddress = 0xD3d47348442cD6e1b3ca1481F26743A93c5ca537;
        devFeeBps = 500; // 5% default
        _unlocked = 1; // Initialize reentrancy guard
    }

    /**
     * @notice Required by the UUPS standard. Restricts upgrade privileges to onlyDev.
     */
    function _authorizeUpgrade(address newImplementation) internal override onlyDev {}
    ...
    function transferDevOwnership(address _newDev) external onlyDev {
        require(_newDev != address(0), "New dev address cannot be zero address");
        emit DevOwnershipTransferred(devAddress, _newDev);
        devAddress = _newDev;
    }

    function setDevFee(uint256 _newFeeBps) external onlyDev {
        require(_newFeeBps <= BPS_DIVISOR, "Fee too high");
        emit FeeUpdated(devFeeBps, _newFeeBps);
    ...
    function _claimReward(address account) internal {
        ...
        if (reward > 0) {
            ...
            uint256 devFee = (reward * devFeeBps) / BPS_DIVISOR;
